...
#### 🖼️ 图片/素材加载（云端稳定性关键）
- **强烈推荐（长期方案）**：管理平台在上传到 E2B 之前，先把远程图片/视频**下载落盘**到项目的 `public/` 或 `src/assets/`，scene 里只引用本地资源。
- **现阶段允许使用 `http://` 资源**（你们当前资源就是这一类），但要意识到：E2B 预览通常走 HTTPS 反代，浏览器可能会阻止 HTTPS 页面加载 HTTP 图片（Mixed Content），导致加载失败。
  - 若发现加载失败：优先让管理平台做“HTTP→HTTPS 可用性探测 + 代理/落盘”，而不是让 scene 直接依赖外网。
- **现阶段允许使用短时效签名 URL**（带 `timestamp/nonce/sign` 这类），但必须假设它可能在播放中途过期 / 云端网络不可达。
- **强制要求（短期兜底）**：无论远程图是否能加载成功，代码都**不允许因为图片加载失败而崩溃/取消渲染**，必须有失败兜底（例如显示占位、跳过该图片）。
- **强制要求（语义占位）**：所有图片组件必须提供可读语义（至少一个）：`alt` / `title` / `aria-label` / `data-caption`。
  - 目的：当图片加载失败时，模板会渲染“主题配色的默认占位图”，并把这些语义文案显示在占位卡片上，让画面依然像“成片”。
- 如果必须用远程 URL：
  - 优先 HTTPS（能用就用）；
  - 避免依赖 Referer/Cookie（云端更容易被防盗链拦）；
  - 所有远程图片必须具备失败兜底。
